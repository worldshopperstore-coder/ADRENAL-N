...
export default function AppLayout({ activeTab, onTabChange, children, session, onLogout }: LayoutProps) {
  ...
  const openRateEdit = () => {
    setUsdDraft(usdRate.toString());
    setEurDraft(eurRate.toString());
    setRateEditMode(true);
    setRateSaved(false);
  };

  const saveRateEdit = () => {
    const usd = parseFloat(usdDraft) || usdRate;
    const eur = parseFloat(eurDraft) || eurRate;
    setUsdRate(usd);
    setEurRate(eur);
    saveExchangeRates(usd, eur);
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
                  ...
                  <>
                    ...
                    <div className="bg-gray-800/50 rounded-lg border border-gray-700/30 overflow-hidden">
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-700/30 border-b border-gray-700/20">
                        ...
                        <span className="text-[11px] text-gray-400 font-semibold uppercase tracking-wider flex-1">Kurlar</span>
                        {rateSaved ? (
                          <span className="flex items-center gap-1 text-[10px] text-emerald-400 font-semibold">
                            <Check className="w-3 h-3" /> Kaydedildi
                          </span>
                        ) : (
                          <button onClick={openRateEdit} className="flex items-center gap-1 text-[10px] text-gray-500 hover:text-amber-400 transition-colors electron-no-drag">
                      ...
                      <div className="grid grid-cols-2 divide-x divide-gray-700/30">
                        <div className="px-2.5 py-1.5">
                          <label className="text-[10px] text-gray-500 mb-0.5 block">USD / TL</label>
                          <span className="font-bold text-amber-400 text-xs">{usdRate.toFixed(2)} ₺</span>
                        </div>
                        <div className="px-2.5 py-1.5">
                          <label className="text-[10px] text-gray-500 mb-0.5 block">EUR / TL</label>
                          <span className="font-bold text-orange-300 text-xs">{eurRate.toFixed(2)} ₺</span>
                        </div>
                      </div>
                    </div>
                  </>
      ...
      {/* Kur Düzenleme Modal — sidebar dışında, hover ile ilgisi yok */}
      {rateEditMode && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-sm p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-amber-400" />
                <h3 className="text-base font-bold text-white">Günlük Kurlar</h3>
              </div>
              <button onClick={() => setRateEditMode(false)} className="p-1.5 rounded-lg hover:bg-gray-800 text-gray-400 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <input
                  ...
                  autoFocus
                  className="w-full px-3 py-2.5 bg-gray-800 border border-amber-500/40 rounded-xl font-bold text-amber-400 text-sm outline-none focus:border-amber-400"
                />
              </div>
              <div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-amber-600 to-orange-500 hover:from-amber-500 hover:to-orange-400 text-white font-bold py-2.5 rounded-xl text-sm flex items-center justify-center gap-2 transition-all"
              >
                <Check className="w-4 h-4" /> Kaydet
              </button>
              <button
                onClick={() => setRateEditMode(false)}
                className="px-4 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white py-2.5 rounded-xl text-sm border border-gray-700 transition-colors"
              >
                İptal
              </button>
            </div>
